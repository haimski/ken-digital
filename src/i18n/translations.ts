const translations = {
    en: {
        profile: {
            title: "Digital Portfolio",
            description: "Welcome to my digital portfolio. With over 20 years of experience in digital solutions and productions, I have partnered with a wide range of companies to drive their success through innovative technology. My expertise spans digital strategy, software development, and process automation, helping businesses of all sizes harness the power of digital tools to streamline operations, boost efficiency, and unlock new growth opportunities. Embrace the future—empower your business with digital excellence.",
        },
        services: {
            title: "Services",
            description: "Explore the range of services I offer to help your business grow.",
            list: [
                {
                    text: "Software Solutions",
                    description: "Custom software development tailored to your business needs. From web and mobile applications to complex enterprise systems, I deliver robust, scalable, and secure solutions that streamline your operations, enhance productivity, and support your long-term growth. My approach ensures seamless integration with your existing workflows and a focus on user experience, empowering your team to achieve more with technology."
                },
                {
                    text: "AI Consultancy",
                    description: "Expert guidance on integrating artificial intelligence into your operations. I help you identify opportunities for AI-driven automation, data analysis, and decision-making, ensuring your business stays ahead of the curve. From strategy to implementation, I provide hands-on support to maximize the value of AI, improve efficiency, and unlock new insights that drive innovation and competitive advantage."
                },
                {
                    text: "Business Automation",
                    description: "Streamline processes and increase efficiency with automation tools. I analyze your current workflows to identify repetitive tasks and bottlenecks, then design and implement automation solutions that save time, reduce errors, and free up your team for higher-value work. My solutions are tailored to your unique needs, ensuring a smooth transition and measurable improvements in productivity and cost savings."
                },
                {
                    text: "Digital Marketing",
                    description: "Grow your online presence with targeted digital marketing strategies. I develop comprehensive campaigns that leverage SEO, social media, content marketing, and analytics to reach your audience and achieve your business goals. By combining creative storytelling with data-driven insights, I help you build brand awareness, engage customers, and drive sustainable growth in the digital landscape."
                },
                {
                    text: "Software Training",
                    description: "Professional training sessions to empower your team with the latest technologies. I offer customized workshops and hands-on learning experiences that cover a wide range of software tools and best practices. Whether you need to upskill your staff or onboard new technologies, my training programs ensure your team is confident, capable, and ready to leverage digital solutions for maximum impact."
                }
            ]
        },
        close: "Close",
        projects: {
            title: "Projects",
            description: "Discover some of the recent projects I’ve worked on with leading companies.",
            list: [
                {
                    title: "Davidson Institute of Science",
                    description: "Developed an interactive educational platform for science outreach and engagement."
                },
                {
                    title: "Weizmann Institute",
                    description: "Built custom research management tools to support scientific collaboration."
                },
                {
                    title: "Teva",
                    description: "Implemented digital solutions for pharmaceutical process optimization."
                },
                {
                    title: "Osem",
                    description: "Created a supply chain automation system for a major food manufacturer."
                },
                {
                    title: "Dance Academy of Jerusalem",
                    description: "Designed a digital registration and scheduling platform for students."
                },
                {
                    title: "Insurance",
                    description: "Developed a claims management system to streamline insurance operations."
                },
                {
                    title: "Software",
                    description: "Delivered bespoke software products for various business needs."
                },
                {
                    title: "Startup",
                    description: "Provided end-to-end technical support for early-stage startups."
                },
                {
                    title: "Tel Aviv University",
                    description: "Built an online portal for academic administration and student services."
                },
                {
                    title: "Affiliates Portal",
                    description: "Launched a comprehensive portal for managing affiliate partnerships."
                },
                {
                    title: "Gaming Competition",
                    description: "Engineered a real-time platform for hosting online gaming tournaments."
                }
            ]
        },
        contact: {
            title: "Contact Me",
            description: "Fill out the form below and I’ll get back to you soon.",
            name: "Name",
            namePlaceholder: "Enter your name",
            email: "Email",
            emailPlaceholder: "Enter your email address",
            phone: "Phone",
            phonePlaceholder: "Enter your phone number",
            company: "Company",
            companyPlaceholder: "Enter your company name",
            role: "Role",
            rolePlaceholder: "Enter your role",
            details: "Details",
            detailsPlaceholder: "Enter additional details",
            submit: "Send",
            text: "Let’s get in touch. Email: expert@example.com",
            invalidEmail: "Please enter a valid email address.",
            required: "This field is required.",
            invalidPhone: "Please enter a valid phone number.",
        },
        nav: {
            profile: "profile",
            services: "services",
            projects: "projects",
            contact: "contact",
        },
        expert: "your digital expert",
        toggle: "עברית",
    },
    he: {
        profile: {
            title: "תיק דיגיטלי",
            description: "ברוכים הבאים לתיק הדיגיטלי שלי. עם ניסיון של מעל 20 שנה בפתרונות והפקות דיגיטליות, עבדתי עם מגוון רחב של חברות כדי להוביל אותן להצלחה באמצעות טכנולוגיה חדשנית. המומחיות שלי כוללת אסטרטגיה דיגיטלית, פיתוח תוכנה ואוטומציה עסקית, ומסייעת לעסקים בכל סדר גודל למנף כלים ותהליכים דיגיטליים לשיפור תפעול, הגדלת יעילות ופתיחת אפשרויות צמיחה חדשות. העתיד כבר כאן—העצימו את העסק שלכם עם מצוינות דיגיטלית.",
        },
        services: {
            title: "שירותים",
            description: "גלו את מגוון השירותים שאני מציע כדי לקדם את העסק שלכם.",
            list: [
                {
                    text: "פתרונות תוכנה",
                    description: "פיתוח תוכנה מותאם אישית לצרכי העסק שלכם. אני מספק פתרונות מקיפים לאפליקציות ווב ומובייל, מערכות ארגוניות מורכבות ועוד, תוך דגש על אבטחה, יציבות, חוויית משתמש ואינטגרציה מלאה עם מערכות קיימות. הפתרונות שלי מסייעים לייעל תהליכים, להגדיל את הפרודוקטיביות ולהניע את הצמיחה של העסק שלכם באמצעות טכנולוגיה מתקדמת."
                },
                {
                    text: "ייעוץ בינה מלאכותית",
                    description: "הכוונה מקצועית לשילוב בינה מלאכותית בתהליכי העבודה. אני מסייע בזיהוי הזדמנויות לאוטומציה, ניתוח נתונים וקבלת החלטות חכמות, ומלווה אתכם משלב האסטרטגיה ועד ליישום בפועל. יחד נבנה פתרונות AI שמייעלים תהליכים, משפרים תוצאות עסקיות ומעניקים יתרון תחרותי בשוק הדיגיטלי."
                },
                {
                    text: "אוטומציה עסקית",
                    description: "ייעול תהליכים עסקיים והגברת יעילות באמצעות כלי אוטומציה. אני מנתח את תהליכי העבודה שלכם, מזהה משימות חוזרות ומיישם פתרונות אוטומטיים שמפחיתים טעויות, חוסכים זמן ומשאבים, ומאפשרים לצוות להתמקד במשימות החשובות באמת. כל פתרון מותאם אישית לצרכים שלכם ומביא לשיפור מדיד בפרודוקטיביות ובהפחתת עלויות."
                },
                {
                    text: "שיווק דיגיטלי",
                    description: "הגדלת הנוכחות הדיגיטלית עם אסטרטגיות שיווק ממוקדות. אני בונה קמפיינים מקיפים הכוללים SEO, רשתות חברתיות, תוכן שיווקי וניתוח נתונים, כדי להגיע לקהל היעד ולהשיג את מטרות העסק. השילוב בין יצירתיות לניתוח נתונים מאפשר בניית מותג חזק, מעורבות לקוחות וצמיחה מתמשכת בעולם הדיגיטלי."
                },
                {
                    text: "הדרכות תוכנה",
                    description: "הדרכות מקצועיות להעצמת הצוות בטכנולוגיות המתקדמות ביותר. אני מציע סדנאות מותאמות אישית ולמידה מעשית במגוון כלים ושיטות עבודה. ההדרכות מסייעות להטמיע טכנולוגיות חדשות, להעלות את רמת הידע והביטחון של הצוות, ולהבטיח שהעסק שלכם יוכל למצות את מלוא הפוטנציאל הדיגיטלי."
                }
            ]
        },
        close: "סגור",
        projects: {
            title: "פרויקטים",
            description: "הכירו כמה מהפרויקטים האחרונים שביצעתי עם חברות מובילות.",
            list: [
                {
                    title: "מכון דוידסון לחינוך מדעי",
                    description: "פיתוח פלטפורמה אינטראקטיבית ללימודי מדע והנגשת תכנים חינוכיים."
                },
                {
                    title: "מכון ויצמן",
                    description: "בניית כלים דיגיטליים לניהול מחקר ושיתופי פעולה מדעיים."
                },
                {
                    title: "טבע",
                    description: "יישום פתרונות דיגיטליים לשיפור תהליכים בתעשיית התרופות."
                },
                {
                    title: "אסם",
                    description: "פיתוח מערכת אוטומציה לשרשרת האספקה עבור יצרן מזון מוביל."
                },
                {
                    title: "האקדמיה למחול ירושלים",
                    description: "עיצוב מערכת רישום וניהול שיעורים דיגיטלית לתלמידים."
                },
                {
                    title: "ביטוח",
                    description: "פיתוח מערכת לניהול תביעות וייעול תהליכי ביטוח."
                },
                {
                    title: "תוכנה",
                    description: "פיתוח מוצרי תוכנה ייעודיים לצרכים עסקיים מגוונים."
                },
                {
                    title: "סטארטאפ",
                    description: "ליווי טכנולוגי מקצה לקצה לסטארטאפים בשלבים ראשוניים."
                },
                {
                    title: "אוניברסיטת תל אביב",
                    description: "בניית פורטל מקוון לניהול אקדמי ושירותי סטודנטים."
                },
                {
                    title: "פורטל שותפים",
                    description: "השקת פורטל מקיף לניהול שותפויות ואפיליאייטס."
                },
                {
                    title: "תחרות גיימינג",
                    description: "פיתוח פלטפורמה בזמן אמת לניהול תחרויות גיימינג אונליין."
                }
            ]
        },
        contact: {
            title: "צור קשר",
            description: "מלאו את הטופס ואחזור אליכם בהקדם.",
            name: "שם",
            namePlaceholder: "הכניסו את שמכם",
            email: "אימייל",
            emailPlaceholder: "הכניסו את כתובת האימייל שלכם",
            phone: "טלפון",
            phonePlaceholder: "הכניסו את מספר הטלפון שלכם",
            company: "חברה",
            companyPlaceholder: "הכניסו את שם החברה",
            role: "תפקיד",
            rolePlaceholder: "הכניסו את התפקיד שלכם",
            details: "פרטים",
            detailsPlaceholder: "הכניסו פרטים נוספים",
            submit: "שלח",
            text: "בואו ניצור קשר. אימייל: expert@example.com",
            invalidEmail: "אנא הזינו כתובת אימייל תקינה.",
            required: "שדה זה הינו חובה.",
            invalidPhone: "אנא הזינו מספר טלפון תקין.",
        },
        nav: {
            profile: "אודות",
            services: "שירותים",
            projects: "פרויקטים",
            contact: "צור קשר",
        },
        expert: "המומחה הדיגיטלי שלך",
        toggle: "English",
    },
};

export default translations;